...
tensorflow::GraphDef CreateGraph(){
...
void CreateSession(struct Param * params){
    ...
    params->session = (double *) session;
    params->input_tensor = &input_tensor;
    params->outputs = (double *) &(outputs);
    params->call = (double *) &(feed_gpu_fetch_cpu);
}

void Test(struct Param * params){
    tensorflow::Session * session = (tensorflow::Session *) params->session;
...
int main(int argc, char **argv) {
    ...
    tensorflow::Session * session = (tensorflow::Session *) params.session;
    tensorflow::Tensor * input_tensor = params.input_tensor;
    std::vector<tensorflow::Tensor> * outputs = (std::vector<tensorflow::Tensor> *) params.outputs;
    tensorflow::Session::CallableHandle * call = (tensorflow::Session::CallableHandle *) params.call;
//    Test(&params);


    // run this to initialize the cuda code (i.e. call cudaMalloc)
//    tensorflow::Status runStatus = session->RunCallable(feed_gpu_fetch_cpu, {input_tensor}, &outputs,
//                                                        nullptr);
    tensorflow::Status runStatus;
//    cudaMemcpy(tensorflow::DMAHelper::base(&input_tensor), inputPointer, sizeof(float) * height, cudaMemcpyDeviceToDevice);
//    cudaMemcpy((void **)input_tensor.scalar<float>().m_data, inputPointer, sizeof(float) * height, cudaMemcpyDeviceToDevice);

//    tensorflow::Scope root = tensorflow::Scope::NewRootScope();
//    //    auto input = tensorflow::ops::Placeholder(root, tensorflow::DT_INT32);
//    auto out = tensorflow::ops::Add(root, input_tensor, {41});
    //    tensorflow::Status s = session->Run({ {input_tensor, {a} } }, {out}, &outputs);
...
//    auto DMAHelper = new tensorflow::DMAHelper;
//    void* d_test = tensorflow::DMAHelper::base(&input_tensor);
//    d_c = input_tensor.flat<float>().data();
    multiplyPointer(d_a, d_b, d_c);
//    float *c_tmp = input_tensor.flat<float>().data();
    cudaMemcpy(input_tensor->flat<float>().data(), d_c, size, cudaMemcpyDeviceToDevice);

    runStatus = session->RunCallable(*call, {*input_tensor}, outputs,
                                                        nullptr);

    cudaMemcpy(d_c, input_tensor->flat<float>().data(), size, cudaMemcpyDeviceToDevice);

    multiplyPointer(d_a, d_c, d_c);
...
//    cudaMemcpy(d_c, input_tensor.flat<float>().data(), size, cudaMemcpyDeviceToDevice);
    ...
    float *c_test = &((*outputs)[0].scalar<float>()());
//    float *c_test = &(outputs[0].scalar<float>()());
    multiplyPointer(d_a, c_test, d_c);

    cudaMemcpy(&c, c_test, size, cudaMemcpyDeviceToHost);
    printf("c is %f \n", c);
